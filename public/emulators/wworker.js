...
function emsc_extract_bundle_to_fs(){ Module.FS.chdir("/home/web_user"); for (const bytes of Module.bundles) { const buffer = Module._malloc(bytes.length); Module.HEAPU8.set(bytes, buffer); const retcode = Module._zip_to_fs(buffer, bytes.length); Module._free(buffer); if (retcode !== 0) { Module.err("Unable to extract bundle archive\n"); return; } try { Module.FS.readFile("/home/web_user/.jsdos/dosbox.conf"); } catch (e) { Module.err("Broken bundle, .jsdos/dosbox.conf not found"); return; } Module.fsCreatedAt = Module.fsCreatedAt || Date.now(); const configContentPtr = Module._getConfigContent(); const configContent = Module.UTF8ToString(configContentPtr); Module._free(configContentPtr); Module.sendMessage("ws-config", { content: configContent }); } delete Module.bundles; }
function emsc_pack_fs_to_bundle(){ Module.FS.chdir("/home/web_user"); const ptr = Module._zip_from_fs(Module.fsCreatedAt); if (ptr === 0) { Module.err("Can't create zip, see more info in logs"); Module._abort(); return; } const length = Module.HEAPU32[ptr / 4]; const memory = Module.HEAPU8; const archive = memory.slice(ptr + 4, ptr + 4 + length); Module._free(ptr); Module.persist(archive); }
...
function emsc_ws_client_sound_init(freq){ Module.sendMessage("ws-sound-init", { freq : freq }); }
function emsc_ws_client_sound_push(samples,num_samples){ if (num_samples > 0) { Module.sendMessage("ws-sound-push", { samples: Module.HEAPF32.slice(samples / 4, samples / 4 + num_samples) }); } }
function emsc_ws_exit_runtime(){ Module.exit = function() { Module.sendMessage("ws-exit"); }; }
function getMTimeMs(path){ var lookup = FS.lookupPath(UTF8ToString(path)); return lookup.node.timestamp; }
...
var asmLibraryArg = {
  ...
  "destroySyncSleep": destroySyncSleep,
  "emsc_add_frame_line": emsc_add_frame_line,
  "emsc_end_frame_update": emsc_end_frame_update,
  "emsc_exit_runtime": emsc_exit_runtime,
  "emsc_extract_bundle_to_fs": emsc_extract_bundle_to_fs,
  "emsc_pack_fs_to_bundle": emsc_pack_fs_to_bundle,
  "emsc_start_frame_update": emsc_start_frame_update,
  "emsc_ws_client_frame_set_size": emsc_ws_client_frame_set_size,
  "emsc_ws_client_sound_init": emsc_ws_client_sound_init,
  "emsc_ws_client_sound_push": emsc_ws_client_sound_push,
  "emsc_ws_exit_runtime": emsc_ws_exit_runtime,
  ...
  "fd_seek": _fd_seek,
  "fd_write": _fd_write,
  "getMTimeMs": getMTimeMs,
  "getpwnam": _getpwnam,
  "initSyncSleep": initSyncSleep,
  "isNormalState": isNormalState,
  "isWorker": isWorker,
  "jsdos_error": jsdos_error,
  "jsdos_log": jsdos_log,
  "jsdos_warn": jsdos_warn,
  "mktime": _mktime,
  "now": now,
  ...
  "syncSleep": syncSleep,
  "time": _time,
  "ws_client_stdout": ws_client_stdout,
  "ws_init_runtime": ws_init_runtime
};
var asm = createWasm();
/** @type {function(...*):?} */
